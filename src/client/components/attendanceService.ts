...
const API_BASE_URL_MONTHLY = 'https://api.eduflow.uz/api/v1/user/monthly';
const API_BASE_URL_WEEKLY = 'https://api.eduflow.uz/api/v1/user/statistics';
...
export const getWeeklyTimesheetData = async (year: number, month: number, interval: number = 0) => {
  try {
    ...
    console.log('URL запроса (Weekly):', 'https://api.eduflow.uz/api/v1/user/statistics');
    console.log('Токен авторизации (Weekly):', token ? 'Присутствует' : 'Отсутствует');

    const response = await axios.get('https://api.eduflow.uz/api/v1/user/statistics', {
      params: {
